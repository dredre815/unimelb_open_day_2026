...
| D-06 | Configure models and behaviour with validated server environment variables | Operator setup stores tab-scoped model, reasoning, mode, and 2–5 round configuration in `sessionStorage` namespace `unimelb-open-day-2026:session-config:v2`; v1 is deleted when v2 is loaded, saved, or cleared | **Medium:** configuration can be altered client-side and may survive reload/crash recovery | Strict client schema, allowlisted models/efforts, versioned namespace, safe defaults, clear control, shortcut-only access during a debate, never store visitor text there |
...
| D-13 | Exactly two automatically advancing advocate rounds and an automatic integrity reveal | Operator selects 2–5 rounds; messages are revealed at least two seconds apart; the first verdict follows at least three seconds of visible deliberation; visitors explicitly trigger the integrity reveal and clean re-check | **Low to medium:** longer sessions and visitor gates can increase dwell time and live API cost | Default to 2 rounds, show the call estimate in setup, prepare five distinct fallback rounds, make the first verdict visibly non-final, allow reset at every stage, rehearse both target viewports |
